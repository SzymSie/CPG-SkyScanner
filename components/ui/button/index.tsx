import { FaBookOpen, FaUserCircle, FaBars } from "react-icons/fa";

export default function Button({
  children,
  className,
  title,
  onClick = () => {},
}) {
  const defaultClassNames = "shadow-md focus:cursor-pointer";
  return (
    <button
      className={`${defaultClassNames} ${className ? className : ""}`}
      onClick={onClick}
      title={title}
    >
      {children}
    </button>
  );
}

export function ButtonHome({ handleClick = () => {}, ...rest }) {
  return (
    <Button
      className="flex items-center bg-slate-900 text-xl border border-transparent rounded-lg px-2.5 py-2 hover:bg-slate-500"
      onClick={handleClick}
      title="Home"
      {...rest}
    >
      <FaBookOpen /> <span className="ml-3">Skyscanner</span>
    </Button>
  );
}

export function ButtonRegional({ handleClick = () => {}, ...rest }) {
  return (
    <Button
      className="items-center bg-slate-900 text-xl border border-transparent rounded-lg px-2.5 py-2 hover:bg-slate-500"
      onClick={handleClick}
      title="Regional"
      {...rest}
    >
      <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 24 24"
        fill="currentColor"
        className="w-6 h-6"
      >
        <path
          fillRule="evenodd"
          d="M12 2.25c-5.385 0-9.75 4.365-9.75 9.75s4.365 9.75 9.75 9.75 9.75-4.365 9.75-9.75S17.385 2.25 12 2.25zM8.547 4.505a8.25 8.25 0 1011.672 8.214l-.46-.46a2.252 2.252 0 01-.422-.586l-1.08-2.16a.414.414 0 00-.663-.107.827.827 0 01-.812.21l-1.273-.363a.89.89 0 00-.738 1.595l.587.39c.59.395.674 1.23.172 1.732l-.2.2c-.211.212-.33.498-.33.796v.41c0 .409-.11.809-.32 1.158l-1.315 2.191a2.11 2.11 0 01-1.81 1.025 1.055 1.055 0 01-1.055-1.055v-1.172c0-.92-.56-1.747-1.414-2.089l-.654-.261a2.25 2.25 0 01-1.384-2.46l.007-.042a2.25 2.25 0 01.29-.787l.09-.15a2.25 2.25 0 012.37-1.048l1.178.236a1.125 1.125 0 001.302-.795l.208-.73a1.125 1.125 0 00-.578-1.315l-.665-.332-.091.091a2.25 2.25 0 01-1.591.659h-.18c-.249 0-.487.1-.662.274a.931.931 0 01-1.458-1.137l1.279-2.132z"
          clipRule="evenodd"
        />
      </svg>
    </Button>
  );
}
export function ButtonLogin({ handleClick = () => {}, ...rest }) {
  return (
    <Button
      className="flex items-center bg-slate-900 text-xl border border-transparent rounded-lg px-2.5 py-2 hover:bg-slate-500"
      onClick={handleClick}
      title="Login"
      {...rest}
    >
      <FaUserCircle /> <span className="ml-3 text-lg">Log in</span>
    </Button>
  );
}

export function ButtonBurger({ handleClick = () => {}, ...rest }) {
  return (
    <Button
      className="items-center bg-slate-900 text-xl border border-transparent rounded-lg px-2.5 py-2 hover:bg-slate-500"
      onClick={handleClick}
      title="Expandable Menu"
      {...rest}
    >
      <FaBars />
    </Button>
  );
}

export function ButtonDeparture() {}

export function ButtonDestination() {}

export function ButtonDate() {}

export function ButtonPeople() {}

export function ButtonSearch() {}
